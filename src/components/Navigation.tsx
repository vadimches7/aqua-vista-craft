      ...
      <div className="container">
        <nav className="flex items-center justify-between h-20">
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link, index) => <a key={index} href={link.href} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
                {link.label}
              </a>)}
          </div>
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && <div className="lg:hidden absolute top-20 left-0 right-0 bg-background/95 backdrop-blur-lg border-b border-border/50 py-6">
            <div className="container flex flex-col gap-4">
              {navLinks.map((link, index) => <a key={index} href={link.href} className="text-foreground py-2" onClick={() => setIsMobileMenuOpen(false)}>
                  {link.label}
                </a>)}
